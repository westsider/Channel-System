...
class MarketCondition: Object {
    ...
    func getMarketCondition(debug:Bool, completion: @escaping (Bool) -> Void) {
        ...
        DispatchQueue.global(qos: .background).async {
            done = self.calcMarketCondFirstRun(debug: debug) // needs a completion handler
            _ = SpReturns().textForStats(yearEnding: 2007)
            _ = SpReturns().textForStats(yearEnding: 2017)
            if done {
                //let mc = self.getData()
                DispatchQueue.main.async {
                    print("Market Condition Complete")
                    completion(true )
                }
            }
        }
    ...
    func calcMarketCondFirstRun(debug:Bool)->Bool {
        deleteAll()
